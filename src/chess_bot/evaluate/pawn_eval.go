...
func PawnEval(cb board.ChessBoard, pawn_cap_rays *[2][64]board.Bitboard) float64 { 
	...
	for _, ind := range wp.Index() {
		...
		if (bp & (adj_files_past)) == 0 {
			if blocked {
				score += past_pawn_val * math.Pow( float64(6 - rows_away), 2) / 2
			} else {
				score += past_pawn_val * math.Pow( float64(6 - rows_away), 2)
			}
		}
	...
	score += chain_val * float64(chains.Count())
	...
	// ------------------------------------------------------------------------
	score += pawn_centre_control(wp, white_attacks, bp, black_attacks)
...
func pawn_centre_control(wp board.Bitboard, wp_cap board.Bitboard,
						bp board.Bitboard, bp_cap board.Bitboard) float64 {
	var score float64 = 0.0
